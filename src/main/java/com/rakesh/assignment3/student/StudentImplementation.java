...
public class StudentImplementation {
    ...
    public static MaleAndFemaleContainer maleAndFemaleCounts(List<Student1> studentList) {
    ...
    public static List<String> getNames(List<Student1> studentList) {
    ...
    public static GradeLevelContainer countBasedOnGradeLevel(List<Student1> studentList, int gradeLevel) {
        long count = studentList.stream().filter(s -> s.getGradeLevel() == gradeLevel).count();
        return new GradeLevelContainer(count);
    }

    public static GradeLevelContainer getAllGradeCount(List<Student1> studentList) {
        Map<Integer, Long> result = studentList.stream().collect(Collectors.groupingBy(Student1::getGradeLevel, Collectors.counting()));
    ...
    public static ActivityContainer countBasedOnActivity(List<Student1> studentList, String activity) {
        long count = studentList.stream().filter(s -> s.getActivities().contains(activity)).count();
        return new ActivityContainer(count);
    }

    public static ActivityContainer ActivityGroupMap(List<Student1> studentList) {
    ...
    public static PerformanceContainer countByLevel(List<Student1> studentList, String level) {
        long count = 0l;
    ...
    public static PerformanceContainer getGpaLevelMap(List<Student1> studentList) {
        Map<String, Long> result = new HashMap<>();
