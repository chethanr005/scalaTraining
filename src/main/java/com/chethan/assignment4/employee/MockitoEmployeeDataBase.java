...
public class MockitoEmployeeDataBase {
    ...
    public static CompletableFuture<Employee> updateEmployeeByIdInMockito(int sid, String columnName, String value) {
        return getEmployeesList().thenApply(employeesList -> employeesList.stream().filter(employee -> employee.getId() == sid).map(IdEmployee -> {
